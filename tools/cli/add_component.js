...
const context = {
    ...
    CSS_MIXINS_PATH: featureName ? '../../styles/mixins.scss' : '../styles/mixins.scss',
};
...
console.log('Create component sass');
targetPath = `${targetDir}/${componentName}.scss`;
tpl = helpers.readTemplate('Component.scss');
toSave(targetPath, helpers.processTemplate(tpl, context));
...
/* ==== Add to style.scss ==== */
console.log('Add entry to style.scss');
targetPath = path.join(targetDir, 'style.scss');
lines = helpers.getLines(targetPath);
i = helpers.lastLineIndex(lines, '@import ');
lines.splice(i + 1, 0, `@import './${componentName}.scss';`);
toSave(targetPath, lines);
